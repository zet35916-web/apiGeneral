...
// Vive en config.respaldos — independiente de sync.respaldos, aunque
// use el mismo mecanismo. Un guardado aquí nunca afecta al otro módulo.
async function guardar({ usuarioId, contenido }) {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO config.respaldos (usuario_id, version, contenido, guardado_en)
       SELECT usuario_id, 1, contenido, guardado_en
       FROM config.respaldos
...
async function obtenerVersion({ usuarioId, version }) {
  const { rows } = await pool.query(
    'SELECT contenido, guardado_en FROM config.respaldos WHERE usuario_id = $1 AND version = $2',
    [usuarioId, version]
  );
  return rows[0] || null;
}
